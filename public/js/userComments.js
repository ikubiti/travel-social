...
tripCommentsEl.addEventListener('click', async (event) => {
  ...
  commentStat = targetElement.getAttribute('data-number');
  if (elementId === 'delete') {
    await deleteComment(commentStat);
  } else {
    let index = targetElement.getAttribute('data-comment');
    let location = targetElement.getAttribute('data-location');
...
const updateComment = async (commentId, index, location) => {
  ...
  updateCommentEl.style.display = 'flex';
  cancelCommentEl.style.display = 'flex';
};
